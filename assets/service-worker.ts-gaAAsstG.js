...
let A = null,
  ...
  C = null;
function L(e) {
  e?.includes("native messaging host not found") && (k = !1);
}
async function R() {
  try {
    return await (async function () {
      ...
      try {
        ...
        const s = [{ name: "com.openai.codexextension", label: "Codex" }];
        for (const a of s)
          try {
  ...
  switch (e.type) {
    case "tool_request":
      await (async function (e) {
        try {
          ...
          if ("execute_tool" === t) {
            if (!n?.tool) return void D(s("No tool specified"));
            const e = n.client_id,
              t = n.args?.tabGroupId,
              r =
                "number" == typeof t
              ...
              i =
                n.args?.tabId ??
                ("browser_batch" === n.tool && Array.isArray(n.args?.actions)
                  ? n.args.actions.find(
              ...
              o =
                "number" == typeof i
                  ? i
                  : ("string" == typeof i && parseInt(i, 10)) || void 0,
              c = n.session_scope;
            D(
              await a({
                toolName: n.tool,
          ...
          } else D({ content: `Unknown method: ${t}` });
        } catch (t) {
          D(
            s(
    ...
    case "status_response":
      M &&
        (clearTimeout(C),
        (C = null),
        M({ nativeHostInstalled: k, mcpConnected: S }),
        (M = null));
      break;
    case "mcp_connected":
      !(async function () {
    ...
    case "mcp_disconnected":
      ((S = !1), l(p.MCP_CONNECTED, !1), t.stopTabGroupChangeListener());
  }
}
...
async function $(e, s) {
  try {
    ...
    if (!a.pathname.startsWith(x)) return !1;
      ...
      return (
        await (async function (e) {
          try {
            ...
            const [e, t] = await Promise.all([R(), r()]);
      ...
      return (
        await (async function (e, s) {
          ...
          try {
            ...
            return (
              h("claude_chrome.extension_url.tab_switch", { success: !1 }),
              await G(s),
              !0
            );
          }
...
async function F() {
  const e = w(),
    t = `claude-browser-extension/${chrome.runtime.getManifest().version} (external)`,
...
async function B() {
  try {
    ...
    for (const n of t)
      try {
        (await I.updateAlarmForPrompt(n), s++);
      } catch (e) {
        a++;
      }
    try {
...
(chrome.runtime.onInstalled.addListener(async (e) => {
  ...
  chrome.runtime.onStartup.addListener(async () => {
    (f(), await F(), await t.initialize(), r(), R(), await B());
  }),
  chrome.permissions.onAdded.addListener((e) => {
    e.permissions?.includes("nativeMessaging") && R();
  }),
  ...
  chrome.notifications.onClicked.addListener(async (e) => {
    ...
    let s = null;
    if (
      (t.length >= 2 && "unknown" !== t[1] && (s = parseInt(t[1], 10)),
      s && !isNaN(s))
    )
      try {
...
async function z(e, s) {
  ...
  if (!n || !n.id || !n.tabs || 0 === n.tabs.length)
  ...
  await t.initialize(!0);
  await t.createGroup(r.id);
  (await l(p.TARGET_TAB_ID, r.id),
    await (async function (e) {
...
(chrome.runtime.onUpdateAvailable.addListener((e) => {
        ...
        if ("SW_KEEPALIVE" !== e.type) {
          ...
          if ("PLAY_NOTIFICATION_SOUND" !== e.type) {
            ...
            if ("logout" === e.type)
              try {
                (await b(),
                  await t.clearAllGroups(),
                  await T(),
                  a({ success: !0 }));
              } catch (n) {
                a({ success: !1, error: String(n) });
              }
                  ...
                  ? (C && clearTimeout(C),
                    new Promise((e) => {
                      ((M = e),
                        ...
                        (C = setTimeout(() => {
                          ((M = null),
                            e({ nativeHostInstalled: k, mcpConnected: S }));
                        }, 1e4)));
                    }))
                  : { nativeHostInstalled: k, mcpConnected: S };
              })();
              a({
                status: {
              ...
              if (s.tab)
                return void a({ success: !1, error: "Forbidden sender" });
              try {
                          ...
                          try {
                            ...
                            const n = await chrome.tabs.query({ groupId: s }),
                              r = async (t) => {
                                ...
                                const s = n[t];
                                if (s.id === e || !s.id)
                                  return void (await r(t + 1));
                                const i = s.id,
                                  o = Date.now(),
                                  c = H.get(i);
                                if (c && o - c.timestamp < 3e3)
                                  return c.isAlive
                                    ? void a({ success: !0 })
                                    : void (await r(t + 1));
                                chrome.runtime.sendMessage(
                                  ...
                                  async (e) => {
                                    const s = e?.success ?? !1;
                                    (H.set(i, { timestamp: o, isAlive: s }),
                                      s ? a({ success: !0 }) : await r(t + 1));
                                  },
                                );
                              };
                            await r(0);
  ...
  chrome.webNavigation.onBeforeNavigate.addListener(async (e) => {
    0 === e.frameId && (await $(e.url, e.tabId));
  }),
  chrome.alarms.onAlarm.addListener(async (e) => {
    if (e.name.startsWith("prompt_"))
      try {
        const n = e.name,
          r = await chrome.storage.local.get(["savedPrompts"]),
          i = (r.savedPrompts || []).find((e) => e.id === n);
        if (i) {
          ...
          try {
            ...
            try {
              await chrome.notifications.create({
                ...
                message: `Task "${i.command || "Scheduled Task"}" failed to execute. ${r.message}`,
                priority: 2,
              });
            } catch (s) {}
          }
      ...
      try {
        const a = e.name.replace("retry_", ""),
          ...
          r = (n.savedPrompts || []).find((e) => e.id === a);
        if (r && ("monthly" === r.repeatType || "annually" === r.repeatType))
          try {
            await I.updateAlarmForPrompt(r);
          } catch (t) {
            try {
              await chrome.notifications.create({
                type: "basic",
                ...
                message: `Could not automatically reschedule "${r.command || "Scheduled Task"}". Please edit the task to reschedule it.`,
                priority: 2,
              });
            } catch (s) {}
          }
  ...
  chrome.runtime.onMessageExternal.addListener(
    (e, t, s) => (
      (async () => {
        ...
        if (
          (a = t.origin) &&
          ["https://chatgpt.com", "https://openai.com"].includes(a)
        ) {
          if ("oauth_redirect" === e.type) {
            const a = await v(e.redirect_uri, t?.tab?.id);
            (s(a), a.success && (E().then(() => r()), R()));
          } else if ("ping" === e.type) s({ success: !0, exists: !0 });
          else if ("onboarding_task" === e.type)
            chrome.runtime.sendMessage(
              { type: "POPULATE_INPUT_TEXT", prompt: e.payload?.prompt },
              (e) => {
                const t = chrome.runtime.lastError?.message;
                s({
                  success: !t && !!e?.success,
                  error: t ?? (e?.success ? void 0 : "side panel not ready"),
                });
              },
            );
