(function () {
  ...
  const ANIMATION_STYLE_ID = "claude-agent-animation-styles";
  const PHANTOM_CURSOR_CONTAINER_ID = "claude-phantom-cursor";
  const AGENT_GLOW_BORDER_ID = "claude-agent-glow-border";
  const AGENT_STOP_CONTAINER_ID = "claude-agent-stop-container";
  const AGENT_STOP_BUTTON_ID = "claude-agent-stop-button";
  const STATIC_INDICATOR_CONTAINER_ID = "claude-static-indicator-container";
  const STATIC_INDICATOR_CHAT_BUTTON_ID = "claude-static-chat-button";
  const STATIC_INDICATOR_CHAT_TOOLTIP_ID = "claude-static-chat-tooltip";
  const STATIC_INDICATOR_CLOSE_BUTTON_ID = "claude-static-close-button";
  const STATIC_INDICATOR_CLOSE_TOOLTIP_ID = "claude-static-close-tooltip";

  const CURSOR_TRANSITION_MS = 180;
  const CURSOR_TRANSITION_TIMEOUT_MS = 220;
  const OVERLAY_FADE_OUT_MS = 300;
  const HEARTBEAT_INTERVAL_MS = 5_000;

  const PHANTOM_CURSOR_PATH = "M0 0 L0 18 L4.5 14 L7.5 21.5 L11 20 L8 13 L14 13 Z";

  class AgentVisualIndicatorController {
    constructor() {
      this.agentGlowBorder = null;
      this.agentStopToolbar = null;
      this.staticIndicator = null;
      this.phantomCursorContainer = null;
      this.phantomCursorAccentLayer = null;
      this.lastCursorPosition = { x: null, y: null };
      this.audioContext = null;
      this.agentIndicatorsVisible = false;
      this.staticIndicatorVisible = false;
      this.agentIndicatorsVisibleBeforeToolUse = false;
      this.staticIndicatorVisibleBeforeToolUse = false;
      this.staticHeartbeatId = null;
      this.isMcpSession = false;
    ...
    ensureAnimationStyles() {
      ...
      style.textContent = `
        @keyframes claude-pulse {
          0% {
            box-shadow:
              inset 0 0 10px rgba(96, 165, 250, 0.32),
              inset 0 0 20px rgba(96, 165, 250, 0.18),
              inset 0 0 30px rgba(96, 165, 250, 0.08);
          }
          50% {
            box-shadow:
              inset 0 0 15px rgba(96, 165, 250, 0.45),
              inset 0 0 25px rgba(96, 165, 250, 0.25),
              inset 0 0 35px rgba(96, 165, 250, 0.12);
          }
          100% {
            box-shadow:
              inset 0 0 10px rgba(96, 165, 250, 0.32),
    ...
    createCursorSvg(id, strokeColor, fillColor, extraCss) {
      const cursorSvg = document.createElementNS(SVG_NS, "svg");
      cursorSvg.id = id;
      cursorSvg.setAttribute("width", "20");
      cursorSvg.setAttribute("height", "26");
      cursorSvg.setAttribute("viewBox", "0 0 20 26");
      cursorSvg.style.cssText = `position:absolute; top:0; left:0; overflow:visible; ${extraCss}`;

      const createPath = (attributes) => {
        const path = document.createElementNS(SVG_NS, "path");
        path.setAttribute("d", PHANTOM_CURSOR_PATH);
      ...
      cursorSvg.appendChild(createPath({ fill: fillColor }));

      return cursorSvg;
    }

    createPhantomCursor(x, y) {
      const container = document.createElement("div");
      container.id = PHANTOM_CURSOR_CONTAINER_ID;
      ...
      const plainLayer = this.createCursorSvg("claude-phantom-cursor-plain", "white", "#111", "");
      const accentLayer = this.createCursorSvg(
        "claude-phantom-cursor-styled",
        "#60A5FA",
        "#0d0d0d",
        "filter: drop-shadow(0 0 4px rgba(96,165,250,0.85)) drop-shadow(0 0 10px rgba(96,165,250,0.35));",
      );

      container.appendChild(plainLayer);
      container.appendChild(accentLayer);

      this.phantomCursorContainer = container;
      this.phantomCursorAccentLayer = accentLayer;
      document.body.appendChild(container);
    }

    async updatePhantomCursor(x, y) {
      if (!this.agentIndicatorsVisible) {
        return;
      }
      ...
      if (!this.phantomCursorContainer) {
        this.createPhantomCursor(x, y);
        return;
      }

      this.phantomCursorContainer.style.transform = `translate3d(${x}px, ${y}px, 0)`;

      if (samePosition || document.hidden) {
        return;
      }
      ...
      await new Promise((resolve) => {
        ...
        const finish = () => {
          ...
          settled = true;
          this.phantomCursorContainer?.removeEventListener("transitionend", finish);
          resolve();
        };

        this.phantomCursorContainer.addEventListener("transitionend", finish, {
          once: true,
        });
        setTimeout(finish, CURSOR_TRANSITION_TIMEOUT_MS);
      });
    }

    removePhantomCursor() {
      if (this.phantomCursorContainer?.parentNode) {
        this.phantomCursorContainer.parentNode.removeChild(this.phantomCursorContainer);
      }
    ...
    createAgentGlowBorder() {
      const border = document.createElement("div");
      border.id = AGENT_GLOW_BORDER_ID;
    ...
    createStopToolbar() {
      const container = document.createElement("div");
      container.id = AGENT_STOP_CONTAINER_ID;
      container.style.cssText = `
        position: fixed;
        bottom: 16px;
        left: 50%;
      ...
      container.innerHTML = `
        <button id="${AGENT_STOP_BUTTON_ID}" style="
          position: relative;
          ...
          justify-content: center;
          box-shadow:
            0 40px 80px rgba(96, 165, 250, 0.14),
            0 4px 14px rgba(96, 165, 250, 0.14);
        ...
        ">
          ...
          <span style="vertical-align: middle;">Stop Codex</span>
        </button>
      `;

      const button = container.querySelector(`#${AGENT_STOP_BUTTON_ID}`);
      if (button) {
        button.addEventListener("mouseenter", () => {
          ...
          button.style.boxShadow =
            "0 40px 80px rgba(96, 165, 250, 0.16), 0 4px 14px rgba(96, 165, 250, 0.16)";
        });
        ...
        button.addEventListener("mouseleave", () => {
          ...
          button.style.boxShadow =
            "0 40px 80px rgba(96, 165, 250, 0.14), 0 4px 14px rgba(96, 165, 250, 0.14)";
        });
    ...
    createStaticIndicator() {
      const container = document.createElement("div");
      container.id = STATIC_INDICATOR_CONTAINER_ID;
      container.innerHTML = `
        ...
        <span style="vertical-align: middle; color: #141413; font-size: 14px; display: inline-block;">Codex is active in this tab group</span>
        ...
        <button id="${STATIC_INDICATOR_CHAT_BUTTON_ID}" style="position: relative; display: inline-flex; align-items: center; justify-content: center; padding: 6px; background: transparent; border: none; cursor: pointer; pointer-events: auto; vertical-align: middle; width: 32px; height: 32px; border-radius: 8px; transition: background 0.2s;">
          ...
          <span id="${STATIC_INDICATOR_CHAT_TOOLTIP_ID}" style="position: absolute; bottom: calc(100% + 12px); left: 50%; transform: translateX(-50%); padding: 6px 12px; background: #30302E; color: #0d0d0d; border-radius: 6px; font-size: 12px; white-space: nowrap; opacity: 0; pointer-events: none; transition: opacity 0.2s; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">Open chat</span>
        </button>
        <button id="${STATIC_INDICATOR_CLOSE_BUTTON_ID}" style="position: relative; display: inline-flex; align-items: center; justify-content: center; padding: 6px; background: transparent; border: none; cursor: pointer; pointer-events: auto; vertical-align: middle; width: 32px; height: 32px; margin-left: 4px; border-radius: 8px; transition: background 0.2s;">
          ...
          <span id="${STATIC_INDICATOR_CLOSE_TOOLTIP_ID}" style="position: absolute; bottom: calc(100% + 12px); left: 50%; transform: translateX(-50%); padding: 6px 12px; background: #30302E; color: #0d0d0d; border-radius: 6px; font-size: 12px; white-space: nowrap; opacity: 0; pointer-events: none; transition: opacity 0.2s; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">Dismiss</span>
        </button>
      `;
      ...
      const chatButton = container.querySelector(`#${STATIC_INDICATOR_CHAT_BUTTON_ID}`);
      const chatTooltip = container.querySelector(`#${STATIC_INDICATOR_CHAT_TOOLTIP_ID}`);
      if (chatButton) {
        chatButton.addEventListener("mouseenter", () => {
          chatButton.style.background = "#111111";
          if (chatTooltip) {
        ...
        chatButton.addEventListener("click", async () => {
          try {
            ...
            // The original bundle ignored this failure path.
          }
        });
      }

      const closeButton = container.querySelector(`#${STATIC_INDICATOR_CLOSE_BUTTON_ID}`);
      const closeTooltip = container.querySelector(`#${STATIC_INDICATOR_CLOSE_TOOLTIP_ID}`);
      if (closeButton) {
        closeButton.addEventListener("mouseenter", () => {
          closeButton.style.background = "#111111";
          if (closeTooltip) {
    ...
    showAgentIndicators(isMcp = false) {
      this.agentIndicatorsVisible = true;
      this.isMcpSession = this.isMcpSession || isMcp === true;

      this.ensureAnimationStyles();

      if (this.agentGlowBorder) {
        this.agentGlowBorder.style.display = "";
      } else {
        this.agentGlowBorder = this.createAgentGlowBorder();
        document.body.appendChild(this.agentGlowBorder);
      }

      if (!this.isMcpSession) {
        if (this.agentStopToolbar) {
          this.agentStopToolbar.style.display = "";
        } else {
          this.agentStopToolbar = this.createStopToolbar();
          document.body.appendChild(this.agentStopToolbar);
        }
      }
      ...
      const cursorX = this.lastCursorPosition.x ?? Math.round(window.innerWidth / 2);
      const cursorY = this.lastCursorPosition.y ?? Math.round(window.innerHeight / 2);
      void this.updatePhantomCursor(cursorX, cursorY);
      if (this.phantomCursorAccentLayer) {
        this.phantomCursorAccentLayer.style.display = "";
      }

      requestAnimationFrame(() => {
        if (this.agentGlowBorder) {
          this.agentGlowBorder.style.opacity = "1";
        }

        if (this.agentStopToolbar) {
          const button = this.agentStopToolbar.querySelector(`#${AGENT_STOP_BUTTON_ID}`);
          if (button) {
    ...
    hideAgentIndicators() {
      ...
      if (this.agentGlowBorder) {
        this.agentGlowBorder.style.opacity = "0";
      }
      ...
      setTimeout(() => {
        ...
        if (this.agentGlowBorder?.parentNode) {
          this.agentGlowBorder.parentNode.removeChild(this.agentGlowBorder);
          this.agentGlowBorder = null;
        }

        if (this.agentStopToolbar?.parentNode) {
          this.agentStopToolbar.parentNode.removeChild(this.agentStopToolbar);
          this.agentStopToolbar = null;
        }

        this.removePhantomCursor();
      }, OVERLAY_FADE_OUT_MS);
    ...
    showStaticIndicator() {
      this.staticIndicatorVisible = true;

      if (this.staticIndicator) {
        this.staticIndicator.style.display = "";
      } else {
        this.staticIndicator = this.createStaticIndicator();
        document.body.appendChild(this.staticIndicator);
      }

      this.startStaticHeartbeat();
    }

    hideStaticIndicator() {
      if (!this.staticIndicatorVisible) {
        return;
      }

      this.staticIndicatorVisible = false;
      this.stopStaticHeartbeat();

      if (this.staticIndicator?.parentNode) {
        this.staticIndicator.parentNode.removeChild(this.staticIndicator);
        this.staticIndicator = null;
      }
    }
    ...
    startStaticHeartbeat() {
      this.stopStaticHeartbeat();
      this.staticHeartbeatId = setInterval(async () => {
        try {
    ...
    stopStaticHeartbeat() {
      if (this.staticHeartbeatId) {
        clearInterval(this.staticHeartbeatId);
        this.staticHeartbeatId = null;
      }
    }

    hideIndicatorsForToolUse() {
      this.agentIndicatorsVisibleBeforeToolUse = this.agentIndicatorsVisible;
      this.staticIndicatorVisibleBeforeToolUse = this.staticIndicatorVisible;

      if (this.agentGlowBorder) {
        this.agentGlowBorder.style.display = "none";
      }

      if (this.agentStopToolbar) {
        this.agentStopToolbar.style.display = "none";
      }

      if (this.phantomCursorAccentLayer) {
        this.phantomCursorAccentLayer.style.display = "none";
      }

      if (this.staticIndicator && this.staticIndicatorVisible) {
        this.staticIndicator.style.display = "none";
      }
    }

    restoreIndicatorsAfterToolUse() {
      if (this.agentIndicatorsVisibleBeforeToolUse) {
        if (this.agentGlowBorder) {
          this.agentGlowBorder.style.display = "";
        }
      ...
      if (this.staticIndicatorVisibleBeforeToolUse && this.staticIndicator) {
        this.staticIndicator.style.display = "";
      }
    ...
    handleRuntimeMessage(message, sender, sendResponse) {
      if (message.type === "SHOW_AGENT_INDICATORS") {
        this.showAgentIndicators(message.isMcp === true);
        sendResponse({ success: true });
        return;
      }

      if (message.type === "HIDE_AGENT_INDICATORS") {
        this.hideAgentIndicators();
        sendResponse({ success: true });
        return;
      }

      if (message.type === "UPDATE_PHANTOM_CURSOR") {
        void this.updatePhantomCursor(message.x, message.y).then(() => {
          sendResponse({ success: true });
        });
        return true;
      }
      ...
      if (message.type === "HIDE_FOR_TOOL_USE") {
        this.hideIndicatorsForToolUse();
        sendResponse({ success: true });
        return;
      }

      if (message.type === "SHOW_AFTER_TOOL_USE") {
        this.restoreIndicatorsAfterToolUse();
        sendResponse({ success: true });
        return;
      }

      if (message.type === "SHOW_STATIC_INDICATOR") {
        this.showStaticIndicator();
        sendResponse({ success: true });
        return;
      }

      if (message.type === "HIDE_STATIC_INDICATOR") {
        this.hideStaticIndicator();
        sendResponse({ success: true });
      }
    }
  ...
  const controller = new AgentVisualIndicatorController();
